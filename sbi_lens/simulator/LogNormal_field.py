...
def shift_fn(params, theta):
    """
    Compute the interpolated shift parameters of the lognormal distribution as
    function of cosmology
    -----------

    params: Jax.numpy.ndarray
    The interpolation table of the shift parameters, with dimensions
    [n_omega_m, n_sigma_8, n_w, n_bins, 4]
...
def fill_shift_array(shifts):
    """
    Calculate the product of the shift parameters between auto and cross
    redshift bins
    -----------
    shifts: Jax.DeviceArray
    Value of the shift parameter of the lognormal distribution with a given
    cosmology for each redshift bin
...
def make_power_map(pk_fn, N, map_size, zero_freq_val=0.0):
    """
    Calculate fourier-space Gaussian fields generated with a given power
    spectrum.
    -----------
    pk_fn:
...
def make_lognormal_power_map(power_map, pshift, zero_freq_val=0.0):
    ...
    The product of the shift parameters of the lognormal distribution with a
    given cosmology, between the auto and cross redshift bin
...
def lensingLogNormal(
    ...
    z = numpyro.sample(
        ...
        dist.MultivariateNormal(
            loc=jnp.zeros((nbins, N, N)), precision_matrix=jnp.eye(N)
        ),
    )
