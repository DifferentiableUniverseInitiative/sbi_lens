...
def fn(params):
  ...
  density_sample_1 = 69.47036304452095 / (
      np.pi * 30.0**2)  #not used when we compute the shift for galaxy sources
  ...
  initialise_new_Universe(a_initial, a_final, omega_m, FLAGS.ob, 0.0,
                          1.0 - omega_m, sigma8, FLAGS.ns, FLAGS.h, w0,
                          FLAGS.wa)
  add_projected_galaxy_sample(0, nz_filename, density_sample_1, b1_sample_1,
                              b2_sample_1, a0, a1)
  ...
  return omega_m, sigma8, w0, bin_id, return_lognormal_shift_for_individual_FLASK_bin(
      theta_in_arcmin, 0, 0)


def main(_):
  # Define the redshift distributions we are going to use, and save them to file for cosmomentum
  parent_nz = jc.redshift.smail_nz(FLAGS.smail_a, FLAGS.smail_b,
                                   FLAGS.smail_z0)
  nz_bins = subdivide(parent_nz,
                      nbins=FLAGS.nbins,
                      zphot_sigma=FLAGS.zphot_sigma)
  ...
  for i in range(FLAGS.nbins):
    ...
    np.savetxt('outputs/shear_photoz_bin_%d.tab' % i,
               row_list,
               header=header,
               delimiter='\t')
