...
_test_params_MASB = [
    {"a": 2.0, "b": 0.25, "c": 0.1, "nb_dimension": 2, "nb_component": 4},
    {"a": 0.01, "b": 0.75, "c": 0.1, "nb_dimension": 3, "nb_component": 5},
    {"a": 1.0, "b": 0.05, "c": 0.99, "nb_dimension": 4, "nb_component": 2},
]
...
def test_bijectorMixtureAffineSigmoidBijector_inverse():
    ...
    for params in _test_params_MASB:
        x = jax.random.uniform(
            jax.random.PRNGKey(0), shape=[batch_size, params["nb_dimension"]]
        )
    ...
    assert_allclose(bij.inverse(bij.forward(x) * 1.0), x, rtol=2e-2, atol=1e-5)
