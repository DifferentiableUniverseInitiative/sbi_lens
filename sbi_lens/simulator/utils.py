...
def get_samples_and_scores(
    ...
    if thetas is None:
        omega_c = jax.vmap(lambda k: trace(seed(model, k)).get_trace()[
            'omega_c']['value'])(keys)
        sigma_8 = jax.vmap(lambda k: trace(seed(model, k)).get_trace()[
            'sigma_8']['value'])(keys)
    ...
    if run_mcmc:
        ...
        def log_prob_fn(params):
            ...
            likelihood = tfd.MultivariateNormalDiag(cell,
                                                    scale_diag=jnp.sqrt(
                                                        np.diag(C)))
            logp = prior.log_prob(params) + likelihood.log_prob(Pl2 -
                                                                cell_noise)
            return logp
