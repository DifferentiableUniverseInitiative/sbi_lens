...
parser.add_argument("--loss", type=str, default='train_compressor_mae')
...
print('######## CREATE AND INITIALIZE COMPRESSOR AND NF ########')

# nf
bijector_layers_compressor = [128] * 2

bijector_compressor = partial(AffineCoupling,
...
if args.resnet == 'resnet34':
  print('ResNet34')
  if args.loss == 'train_compressor_ngll':
    compressor = hk.transform_with_state(lambda y: ResNet34(aug_dim)
                                         (y, is_training=True))
  else:
    compressor = hk.transform_with_state(lambda y: ResNet34(dim)
                                         (y, is_training=True))
elif args.resnet == 'resnet18':
  print('ResNet18')
  if args.loss == 'train_compressor_ngll':
    compressor = hk.transform_with_state(lambda y: ResNet18(aug_dim)
                                         (y, is_training=True))
  else:
    compressor = hk.transform_with_state(lambda y: ResNet18(dim)
                                         (y, is_training=True))
...
# init nf
params_nf = nf.init(jax.random.PRNGKey(0),
...
model_compressor = TrainModel(compressor=compressor,
                              nf=nf,
...
with open(
    DATA_DIR / "params_compressor/params_nd_compressor_{}.pkl".format(l_name),
    "wb") as fp:
