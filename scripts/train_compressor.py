...
m_data = jnp.load(
  DATA_DIR / 'm_data__{}N_{}ms_{}gpa_{}se.npy'.format(
    N,
    map_size,
    gals_per_arcmin2,
    sigma_e
...
if args.resnet == 'resnet34':
  ...
  compressor = hk.transform_with_state(
    lambda y : ResNet34(dim)(y, is_training=True)
  )
...
with open(DATA_DIR / "params_compressor/params_nd_compressor_{}.pkl".format(l_name), "wb") as fp:
  pickle.dump(parameters_compressor, fp)

with open(DATA_DIR / "params_compressor/opt_state_resnet_{}.pkl".format(l_name), "wb") as fp:
