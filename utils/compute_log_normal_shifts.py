...
flags.DEFINE_float("w_max", -0.3, "w max")
...
flags.DEFINE_float("smail_a", 2.0, "smail a parameter")
...
def fn(params):
    ...
    initialise_new_Universe.argtypes = [
        ctypes.c_double,
        ctypes.c_double,
        ctypes.c_double,
        ctypes.c_double,
    ...
    add_projected_galaxy_sample.argtypes = [
        ctypes.c_int,
        ctypes.c_char_p,
        ctypes.c_double,
        ctypes.c_double,
    ...
    return_lognormal_shift_for_individual_FLASK_bin = (
        lib.return_lognormal_shift_for_individual_FLASK_bin
    )
    return_lognormal_shift_for_individual_FLASK_bin.argtypes = [
        ctypes.c_double,
        ctypes.c_int,
        ctypes.c_int,
    ]
    ...
    a_initial = 0.000025
    a_final = 1.0
    density_sample_1 = 69.47036304452095 / (
        np.pi * 30.0**2
    )  # not used when we compute the shift for galaxy sources
    ...
    # Read the filename
    nz_filename = ctypes.c_char_p(
        ("outputs/shear_photoz_bin_%d.tab" % bin_id).encode("utf-8")
    )

    initialise_new_Universe(
        a_initial,
        a_final,
        omega_m,
        FLAGS.ob,
    ...
    add_projected_galaxy_sample(
        0, nz_filename, density_sample_1, b1_sample_1, b2_sample_1, a0, a1
    )
    # Converting from pixel size to scale used in cosmomentum, following
    # Boruah et al. 2021
    ...
    return (
        ...
        w0,
        bin_id,
        return_lognormal_shift_for_individual_FLASK_bin(theta_in_arcmin, 0, 0),
    )


def main(_):
    # Define the redshift distributions we are going to use, and save them
    # to file for cosmomentum
    ...
    for i in range(FLAGS.nbins):
        ...
        np.savetxt(
            "outputs/shear_photoz_bin_%d.tab" % i,
            row_list,
            header=header,
            delimiter="\t",
        )
    ...
    results = np.array(results).reshape(
        [FLAGS.nsteps, FLAGS.nsteps, FLAGS.nsteps, FLAGS.nbins, 5]
    )
